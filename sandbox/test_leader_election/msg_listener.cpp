...
int process_rec_msg(char * acBuffer) {
    ...
    switch (msg.msgType) {
        ...
        case MSG:
        {
            ...
            printf("I AM A SERVER: %i\n", is_server);
            if (strcmp(msg.data.c_str(), "election") == 0) {
                printf("calling initiate_leader_election()\n");
        ...
        case NEW_LEADER_ELECTED: // using SERVER_INFO
        {
            ...
            if (!is_server) {
                ...
                printf("ip port = %s %s\n", &acBuffer[DATA], &acBuffer[iTempIndex]);
                ...
                // Store server's information in the global sServerInfo struct
                sServerInfo.ipAddr = &acBuffer[DATA];
                sServerInfo.port = atoi(&acBuffer[iTempIndex]);

                inet_pton(AF_INET, &acBuffer[DATA], &(sServerAddr.sin_addr));
                sServerAddr.sin_port = htons(atoi(&acBuffer[iTempIndex]));
