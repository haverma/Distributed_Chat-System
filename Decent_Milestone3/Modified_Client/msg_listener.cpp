...
void flush_dead_clients(std::list<int> deadclients);void process_hbq(int timestamp, int message_id, int senderPort);
        ...
        case MSG:
            {
                ...
                sprintf(&acBuffer[MSG_ID], "%d", msg.msgId);
                sprintf(&acBuffer[SEQ_NUM], "%d", 5);
                std::cout<<"Proposing a seq Number: " << 5 <<" for message " << msg.msgId << "\n";
